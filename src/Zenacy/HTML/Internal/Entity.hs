...
entityMatch = Trie.match entityTrie

-- | A trie of the entity data with surrogates converted.
entityTrie :: Trie BS
entityTrie =
  Trie.fromList $ map (f . g) entityData
  where
    f (x, y) = (x, Text.encodeUtf8 $ Text.pack y)
    g (x, y) = case y of
      [a, b] ->
        let a' = ord a
            b' = ord b
        in if isUTF16Surrogate a' b'
              then (x, [convUTF16Surrogate a' b'])
              else (x, [a,b])
      _otherwise ->
        (x, y)
...
convUTF16Surrogate high low =
  chr $ (high - 0xD800) * 0x400 + (low - 0xDC00) + 0x10000

-- | Determines is a pair of codes represent a surrogate.
isUTF16Surrogate :: Int -> Int -> Bool
isUTF16Surrogate high low =
